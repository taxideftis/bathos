...
name: model-switch
description: |
  BATHOS 모델 전환 — 지금 이 세션의 백엔드를 당장 바꾸거나 복귀 (dry-run→확인 1회→apply, 판단은 전부 bathos CLI)
  TRIGGER: 명시 멘션($model-switch)만 — CLAUDE.md §8·커맨드 자체 각주에 자연어 별칭 정의 없음(날조 금지, 미정 상태 정직 표기). 자연어 확장은 실사용 관측 후 사용자 확정을 거쳐 추가한다.
  NOT: 계획(웨이브·역할별 런타임/모델 배정) 변경 의도(→model-config)에는 발동 금지 — 이 스킬은 지금 세션의 백엔드 실황 전환만 담당한다.
---
