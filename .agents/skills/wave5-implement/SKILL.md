...
**구현 규율(사다리) 주입 — 필수.** 위임 프롬프트에 다음 두 가지를 반드시 포함한다:
1. 규율 문서 경로 `.claude/agents/_preamble/ponytail-inject-kr.md`(정본, 보조 번역 `-en`/`-ja`/`-es`)를 먼저 읽고 그 규율에 따라 구현하라는 지시. Codex 위임에는 프리앰블 자동 주입이 없으므로 **경로를 지시문에 명문화**해야 한다(소유 경로 명문화와 동일 이유).
2. 현재 intensity 값 — `jq -r '.intensity // "full"' .agent-team/_state/session-flags.json`로 읽어 프롬프트에 명시. 변경이 필요하면 사용자에게 묻는다(자동 변경 금지).

> **축 분리:** 사다리는 **무엇을 만들지**(스코프·추상화·의존성)를, ETHOS "Boil the Ocean"은 **정한 범위를 얼마나 완전히**(에러 경로·엣지·테스트)를 지배한다. 사다리를 근거로 검증·에러 처리·보안·접근성·테스트를 잘라내는 것은 규율 위반이다(규율 §0·§4).
...
- [ ] **구현 규율(사다리) 준수** — 요청받지 않은 추상화·"나중을 위한" 스캐폴딩 없음. 새 의존성을 추가했다면 사다리 5단을 못 넘은 이유를 `08-impl-notes/`에 한 줄로 남겼는가.
- [ ] **의도적 단순화에 `ponytail:` 마커** — 알려진 천장이 있는 단순화마다 `ponytail: <ceiling>, <upgrade path>`(영어)를 남겼는가. 업그레이드 트리거 없는 마커는 `/bathos-debt`에서 `no-trigger`로 잡힌다.
- [ ] **비자명 로직에 실행 가능한 검증 1개** — 분기·루프·파서·금액/보안 경로에 `assert` 자체검증이나 작은 테스트 하나(자명한 한 줄은 면제).
...
- **요청받지 않은 추상화** — 구현체가 하나뿐인 인터페이스, 산출물이 하나뿐인 팩토리, 아무도 바꾸지 않는 값을 위한 설정(규율 §3)
